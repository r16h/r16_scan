# #!/bin/bash
# ORIGINAL CREATOR: Luca Garofalo (R16 TEAM)
# AUTHOR: Luca Garofalo (R16 TEAM)
# Copyright (C) 2021-2023 R16 TEAM
...
function Preference {
	...
	read -p"$GREEN[#R16 TEAM#]$WHITE-->" Language
	while [ "$Language" == "" ];
		do
		printf "${WHITE}\nSELECT YOUR GUI-DEFAULT LANGUAGE\n(1)ENGLISH\n(2)ITALIANO\n(3)FRANÇAIS\n(4)BROWSER-LANGUAGE\n\n"
		read -p"$GREEN[#R16 TEAM#]$WHITE-->" Language
	...
	read -p"$GREEN[#R16 TEAM#]$WHITE-->" Theme
	while [ "$Language" == "" ];
		do
		printf "${WHITE}\nSELECT YOUR GUI-DEFAULT THEME\n(1)LIGHT\n(2)DARK\n(3)HIGH-CONTRAST\n\n"
		read -p"$GREEN[#R16 TEAM#]$WHITE-->" Language
...
function Mail_Options {
	...
	read -p"$GREEN[#R16 TEAM#]$WHITE-->" Opt
	while [ "$Opt" = "" ];
		do
        printf "${BLUE}\n[?]${WHITE}WOULD YOU LIKE TO ENABLE EMAIL-OPTION(1)YES(2)NO\n\n"
		read -p"$GREEN[#R16 TEAM#]$WHITE-->" Opt
	...
	if [ $Opt == 1 ];
		...
		read -p"$GREEN[#R16 TEAM#]$WHITE-->" recipient
		while [ "$recipient" = "" ];
			do
			printf "${WHITE}\nINSERT YOUR RECIPIENT EMAIL\n\n"
			read -p"$GREEN[#R16 TEAM#]$WHITE-->" recipient
		...
		read -sp"$GREEN[#R16 TEAM#]$WHITE-->" password
		while [ "$password" = "" ];
			do
			printf "${WHITE}\nINSERT YOUR EMAIL PASSWORD\n\n"
			read -sp"$GREEN[#R16 TEAM#]$WHITE-->" password
		...
		read -p"$GREEN[#R16 TEAM#]$WHITE-->" destination
		while [ "$destination" = "" ];
			do
			printf "${WHITE}\nINSERT YOUR DESTINATION EMAIL\n\n"
			read -p"$GREEN[#R16 TEAM#]$WHITE-->" destination
		...
		read -p"$GREEN[#R16 TEAM#]$WHITE-->" server
		while [ "$server" = "" ];
			do
			printf "${WHITE}\nINSERT YOUR SMTP SERVER EX smtp.test.com\n\n"
			read -p"$GREEN[#R16 TEAM#]$WHITE-->" server
		...
		read -p"$GREEN[#R16 TEAM#]$WHITE-->" port
		while [ "$port" = "" ];
			do
...
function Options {
	...
	read -sp"$GREEN[#R16 TEAM#]$WHITE-->" up_pass
	while [ "$up_pass" = "" ];
		do
        printf "${WHITE}\nINSERT YOUR UPDATE-PASSWORD \n\n"
        read -sp"$GREEN[#R16 TEAM#]$WHITE-->" up_pass
	...
	read -p"$GREEN[#R16 TEAM#]$WHITE-->" Token
	while [ "$Token" == "" ];
		do
		printf "${WHITE}\n\nWOULD YOU LIKE TO ACCESS YOUR DATABASE ON OTHER DEVICES(ON THE SAME NETWORK)?(1)YES(2)NO\n\n"
		read -p"$GREEN[#R16 TEAM#]$WHITE-->" Token
	...
	read -p"$GREEN[#R16 TEAM#]$WHITE-->" Access
	while [ "$Access" == "" ];
		do
		printf "${WHITE}\n\nWOULD YOU LIKE TO ADD SOME CREDENTIALS FOR ACCESS THE DATABASE?(1)YES(2)NO\n\n"
		read -p"$GREEN[#R16 TEAM#]$WHITE-->" Access
	...
	read -p"$GREEN[#R16 TEAM#]$WHITE-->" DateFormat
	while [ "$DateFormat" == "" ];
		do
		printf "${WHITE}\nSELECT YOUR DATE-FORMAT\n(1)EUROPE(DD/MM/YYYY)\n(2)AMERICA'USA'(MM/DD/YYYY)\n(3)ASIA(YYYY/MM/DD)\n\n"
		read -p"$GREEN[#R16 TEAM#]$WHITE-->" DateFormat
	...
	cd Configuration
	echo ";THIS FILE HAS BEEN GENERATE BY R16 TEAM INSTALLER">Configuration.ini
...
echo '{
	...
	echo ";THIS FILE HAS BEEN GENERATE BY R16 TEAM INSTALLER">Configuration/Configuration.ini
...
function installer {
	...
	if [ $confvar == 1 ];
		...
		read -p "$GREEN[#R16 TEAM#]$WHITE-->" selected
		while [ "$selected" == "" ];
			do
			printf "${BLUE}\n\nWOULD YOU LIKE TO SET(1)MANUAL-INSTALLATION(2)AUTO-INSTALLATION\n\n"
			read -p "$GREEN[#R16 TEAM#]$WHITE-->" selected
		...
		printf "${LIGHTGREEN}\n\nTHANK YOU FOR HAVE INSTALLED R16 TEAM\n\n"
		exit 0
	fi
